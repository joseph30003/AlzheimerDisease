...
public class Nodes_nomalization {
	...
	public static void metamapUpdate(Connection conn) {
		try
	    {
		  ...
		  String query = "select a.id,a.source,a.source_id from Metamap a inner join (select source,source_id,max(score) max_score from Metamap group by source,source_id ) b on a.source=b.source and a.source_id=b.source_id and a.score=b.max_score";
		  ResultSet rs = conn.createStatement().executeQuery(query);
